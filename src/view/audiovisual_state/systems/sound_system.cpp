...
////////////////////////////////
// Add acoustic sound physics
////////////////////////////////
#include "../../../AcousticBaker.hpp"

struct shouldnt_play {};

void augs::update_multiple_properties::update(
    augs::sound_source_proxy_data &data) {
  data.last_pitch = pitch;
  data.last_gain = gain;
}

std::optional<transformr> sound_system::update_properties_input::find_transform(
    const absolute_or_local &positioning) const {
  return ::find_transform(positioning, get_listener().get_cosmos(), interp);
}

const_entity_handle
sound_system::update_properties_input::get_listener() const {
...
bool sound_system::update_properties_input::short_sound_limit_exceeded() const {
  return static_cast<int>(owner.short_sounds.size()) >=
         settings.max_short_sounds;
}
...
void sound_system::generic_sound_cache::stop_and_free(
...
bool sound_system::start_fading(generic_sound_cache &cache,
                                const float fade_per_sec) {
  if (!container_full(fading_sources)) {
    if (cache.probably_still_playing()) {
      fading_sources.push_back(
          {cache.original.input.id, cache.source, fade_per_sec});
      return true;
    }
  }
...
void sound_system::update_listener(const augs::audio_renderer &renderer,
  ...
  const auto listener_transform =
      listener_handle ? listener_handle.get_viewing_transform(sys)
                      : listener.cone.eye.transform;

  {
    const auto listener_pos = settings.listener_reference ==
                                      listener_position_reference::SCREEN_CENTER
                                  ? world_screen_center
  ...
  cmd.velocity =
      listener_handle ? listener_handle.get_effective_velocity() : vec2(0, 0);
...
sound_system::generic_sound_cache::generic_sound_cache(
    ...
    const packaged_sound_effect &original, const update_properties_input in)
    : source(proxy_id), original(original),
...
sound_system::generic_sound_cache::generic_sound_cache(
    const augs::sound_source_proxy_id proxy_id,
    const packaged_multi_sound_effect &multi, const update_properties_input in)
    : source(proxy_id), positioning(multi.start.positioning),
...
augs::sound_source_proxy sound_system::generic_sound_cache::get_proxy(
    const update_properties_input &in) {
  return {in.renderer, source};
}

void sound_system::generic_sound_cache::bind(const update_properties_input &in,
                                             const augs::sound_buffer &buf) {
  const auto proxy = get_proxy(in);
  proxy.bind_buffer(buf, original.start.variation_number);
}

bool sound_system::generic_sound_cache::rebind_buffer(
    const update_properties_input in) {
...
bool sound_system::generic_sound_cache::should_play(
    const update_properties_input in) const {
  ...
  if (listening_character) {
    ...
    return target_faction == faction_type::SPECTATOR ||
...
void sound_system::generic_sound_cache::update_properties(
    const update_properties_input in) {
  const auto listening_character = in.get_listener();
  ...
  const auto listening_faction =
      (listening_character ? listening_character.get_official_faction()
                           : faction_type::SPECTATOR);
  const auto target_faction = original.start.listener_faction;

  const bool is_direct_listener =
      original.input.modifier.always_direct_listener ||
      (listening_character.alive() &&
       listening_character == original.start.direct_listener) ||
      (listening_character.dead() && listening_faction == target_faction &&
       target_faction != faction_type::SPECTATOR);
  ...
  if (previous_transform && !is_direct_listener &&
    ...
    if (!m.disable_velocity) {
      if (interp_enabled || when_set_velocity != cosm.get_timestamp()) {
        ...
        const auto sq_max = in.settings.max_speed_for_doppler_calculation *
                            in.settings.max_speed_for_doppler_calculation;
        const auto sq_len = cmd.velocity.length_sq();

        if (sq_len > sq_max) {
          cmd.velocity.set_length(
              in.settings.max_speed_for_doppler_calculation);
        }
      }
    }
  ...
  const bool is_nonlinear =
      !is_linear && dist_model != augs::distance_model::NONE;
  ...
  const auto mult_via_settings = [&]() {
    if (original.input.modifier.always_direct_listener) {
      if (const auto buf = source.buffer_meta; buf.is_set()) {
        if (buf.computed_length_in_seconds >
            in.settings.treat_as_music_sounds_longer_than_secs) {
  ...
  if (is_linear) {
    ...
    if (dist > ref_distance) {
      custom_dist_gain_mult *=
          1 - std::clamp(dist - ref_distance, 0.f, max_distance) / max_distance;
    }
  ...
  // ==========================================
  // TACTICAL MIL-SIM AUDIO OCCLUSION INJECTION
  // ==========================================
  if (!is_direct_listener) {
    // 1. Get our positions
    const auto sound_pos = current_transform.pos;
    const auto listener_pos =
        listening_character
            ? listening_character.get_viewing_transform(in.interp).pos
            : in.ear.cone.eye.transform.pos;

    // 2. Convert world floating-point coordinates to our 1D Acoustic Grid
    // indices (Assuming extern vectors and grid dimensions from your Baker
    // script)
    extern std::vector<uint8_t> acoustic_grid;
    extern int grid_width;
    extern int grid_height;
    const float ACOUSTIC_TILE_SIZE = 32.0f;

    int sound_x = static_cast<int>(sound_pos.x / ACOUSTIC_TILE_SIZE);
    int sound_y = static_cast<int>(sound_pos.y / ACOUSTIC_TILE_SIZE);
    int listen_x = static_cast<int>(listener_pos.x / ACOUSTIC_TILE_SIZE);
    int listen_y = static_cast<int>(listener_pos.y / ACOUSTIC_TILE_SIZE);

    // Bound checks to ensure we don't look up memory outside our grid array
    if (sound_x >= 0 && sound_x < grid_width && sound_y >= 0 &&
        sound_y < grid_height && listen_x >= 0 && listen_x < grid_width &&
        listen_y >= 0 && listen_y < grid_height) {

      int sound_idx = (sound_y * grid_width) + sound_x;
      int listen_idx = (listen_y * grid_width) + listen_x;

      uint8_t sound_zone = acoustic_grid[sound_idx];
      uint8_t listen_zone = acoustic_grid[listen_idx];

      // 3. Evaluate the acoustic environmental boundaries
      // Zone 2 = Outdoors, Zone 0 = Indoor pocket (isolated by flood fill)
      if (sound_zone != listen_zone) {
        // One player is inside a building, the other is outside!
        // Heavily muffle the high frequencies, and cut the overall volume by
        // 40%
        cmd.gain *= 0.60f;
        cmd.lowpass_gainhf = std::min(cmd.lowpass_gainhf, 0.25f);
      } else if (sound_zone == 0 && listen_zone == 0) {
        /* Optional Advanced Step: Both are inside!
         I f you want separate buildings* to muffle each other, you can assign
         unique Room IDs during your step-2 flood fill instead of setting every
         room to '0'. If their room IDs don't match, you can apply a medium
         dampening wall modifier here: cmd.gain *= 0.75f; cmd.lowpass_gainhf =
         std::min(cmd.lowpass_gainhf, 0.45f);
         */
      }
    }
  }
  ...
  cmd.gain = std::clamp((1 - flash_mult) * std::clamp(m.gain, 0.f, 1.f) *
                            mult_via_settings * custom_dist_gain_mult,
                        0.f, 1.f);
  ...
  if (!(in.dt == augs::delta::zero)) {
    if (gain_dependent_lifetime) {
      if (elapsed_secs == 0.f) {
        if (custom_dist_gain_mult >
            in.settings.gain_threshold_for_bullet_trace_sounds) {
...
void sound_system::generic_sound_cache::maybe_play_next(
    update_properties_input in) {
...
void sound_system::update_effects_from_messages(
    const const_logic_step step, const update_properties_input in) {
  {
    ...
    for (auto &e : events) {
      {
        ...
        if (!(s == collision_sound_source())) {
          ...
          if constexpr (std::is_same_v<decltype(dummy),
          ...
          if (start.is_missile_impact()) {
            const auto max_occurences =
                in.settings.missile_impact_occurences_before_cooldown;
            ...
            succession.max_ms =
                in.settings.missile_impact_sound_cooldown_duration;
          } else {
            ...
            if (succession.consecutive_occurences >
                start.collision_mute_after_playing_times) {
              // LOG("Skipping");
              continue;
            }
      ...
      if (in.settings.max_short_sounds > 0 && !id_pool.full() &&
          short_sounds.size() < short_sounds.max_size()) {
        ...
        auto release_id = [&]() { id_pool.free(new_id); };
...
void sound_system::advance_flash(const const_entity_handle listener,
                                 const augs::delta dt) {
  if (listener.dead()) {
    return;
  }
...
void sound_system::update_elapsed_times(const augs::delta dt) {
  ...
  auto update_facade = [&](auto &cache) { cache.update_elapsed(dt); };
...
void sound_system::update_sound_properties(const update_properties_input in) {
  ...
  update_listener(in.renderer, in.ear, in.interp, in.settings, screen_center);
...
#else
  cosm.for_each_having<components::gun>([&](const auto gun_entity) {
        ...
        if (!id_pool.full()) {
          ...
          try {
            firearm_engine_caches.try_emplace(
                id, continuous_sound_cache{{new_id, *sound, in},
                                           {gun_entity.get_name()}});
          } catch (const effect_not_found &) {
            release_id();
          } catch (const shouldnt_play &) {
  ...
  cosm.for_each_having<invariants::continuous_sound>(
      [&](const auto sound_entity) {
        ...
        const auto &continuous_sound =
            sound_entity.template get<invariants::continuous_sound>();
        ...
        if (auto *const existing =
                mapped_or_nullptr(continuous_sound_caches, id)) {
          existing->cache.original = sound;
          ...
          if (!id_pool.full()) {
            ...
            try {
              continuous_sound_caches.try_emplace(
                  id, continuous_sound_cache{{new_id, sound, in},
                                             {sound_entity.get_name()}});
            } catch (const effect_not_found &) {
              release_id();
            } catch (const shouldnt_play &) {
              release_id();
  ...
  auto update_facade = [&](auto &cache) {
    ...
    const bool still_playing_or_has_followup =
        cache.probably_still_playing() || cache.followup_inputs.size() > 0;
  ...
  auto reseek_if_diverged = [&](const auto &subject, const auto &cache) {
    ...
    if (const auto buf = source.buffer_meta; buf.is_set()) {
      ...
      if (looping || secs > in.settings.sync_sounds_longer_than_secs) {
        ...
        auto total_secs_passed =
            cosm.get_total_seconds_passed(in.interpolation_ratio);
        ...
        if (looping) {
          ...
          total_secs_passed =
              double(total_steps + in.interpolation_ratio) * in.inv_tickrate;
        }

        const auto total_lived_secs =
            std::max(0.0, (total_secs_passed - born_at_secs) *
                              in.speed_multiplier * m.pitch);
        ...
        if (!looping && total_lived_secs >= secs &&
            total_lived_cycles + 1 >= m.repetitions) {
...
void sound_system::fade_sources(const augs::audio_renderer &renderer,
                                const augs::delta dt) {
